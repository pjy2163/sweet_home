...
| population | LOCAL_PEOPLE_DONG_YYYYMM.zip | 소스 확정 | zip/csv | 서울 열린데이터광장 행정동 단위 서울 생활인구(내국인) |
...
- `population_fact.csv`는 서울 열린데이터광장 `행정동 단위 서울 생활인구(내국인)` 데이터를 원천으로 사용합니다.
