          ...
          >
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${candidateState === "saved" ? "bg-[#eaf4ff] text-[#1888e8]" : candidateState === "excluded" ? "bg-[#f0f1f3] text-[#858b98]" : "bg-[#eef7f3] text-[#3c8065]"}`}>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 border-t border-[#eceef2] pt-3">
              <button className={`rounded-lg px-3 py-2 text-xs font-semibold ${candidateState === "saved" ? "bg-[#eaf4ff] text-[#1888e8]" : "border border-[#dce1e8] text-[#596174]"}`} onClick={() => onCandidateStateChange(region.region_id, "saved")} type="button">
                {candidateState === "saved" ? "저장 취소" : "후보 저장"}
              </button>
              <button className={`rounded-lg px-3 py-2 text-xs font-semibold ${candidateState === "excluded" ? "bg-[#f0f1f3] text-[#777e8c]" : "border border-[#dce1e8] text-[#747b89]"}`} onClick={() => onCandidateStateChange(region.region_id, "excluded")} type="button">
    ...
    <aside className="relative h-full min-h-[420px] overflow-hidden rounded-2xl border border-white/12 bg-[#111827] p-5 text-[#f5f5f7] shadow-[0_12px_36px_rgba(23,32,59,.12)]">
      ...
      <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.04] p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {region.selection_source === "map_click" ? <span className="text-sm leading-6 text-[#cacaca]">클릭한 좌표가 속한 행정동의 데이터입니다. 개별 주소나 건물을 분석한 결과는 아닙니다.</span> : (region.matched_indicators ?? []).length > 0 ? region.matched_indicators?.map((indicator) => <span className="rounded-full border border-[#847dff]/30 bg-[#847dff]/15 px-3 py-1 text-xs font-semibold text-[#d8d6ff]" key={indicator}>{indicator}</span>) : <span className="text-sm text-[#cacaca]">현재 선택한 지표 기준 상위 지역입니다.</span>}
        </div>
      </div>
...
function EvidenceProfileRow({ profile }: { profile: EvidenceProfile }) {
  return <div className="grid grid-cols-[4.5rem_3rem_1fr] items-center gap-3"><div><p className="text-sm font-semibold text-[#f5f5f7]">{profile.label}</p><p className="mt-1 truncate text-[10px] text-[#6a6b6b]" title={profile.detail}>{profile.detail}</p></div><span className={`text-xs font-bold ${profile.caution ? "text-[#ffcf70]" : profile.matched ? "text-[#d8d6ff]" : "text-[#9f9fa0]"}`}>{profile.status}</span><div className="relative h-3 rounded-full bg-white/[0.08]"><div className="absolute left-0 top-1/2 h-px w-full -translate-y-1/2 bg-white/10" /><div className={`absolute top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 ${profile.caution ? "border-[#ffcf70] bg-[#332716]" : profile.matched ? "border-[#d8d6ff] bg-[#847dff]" : "border-[#6a6b6b] bg-[#1f2422]"}`} style={{ left: `${profile.position}%` }} /></div></div>;
}
